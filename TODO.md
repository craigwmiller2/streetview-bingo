# TODO

- [ ] Add interface to download localStorage data
- [ ] Add interface to import data to localStorage for restoring account
- [ ] Create a "Settings" page
- [ ] Move the "Delete data" button from Stats page to Settings page
- [ ] Add Export/Import option for localStorage data on Settings pages
- [ ] Make sure there's a confirmation button for the import that will overwrite existing data
- [ ] Update the layout/look of the Acheivements page. Too wide currently.
- [ ] Add "Data achieved" stat for achievements
- [ ] Add a countdown when you click one of the starting buttons
- [ ] Change achievement "rarity" to more specific categorisation, e.g. Time-based, Location-based, Item-based
